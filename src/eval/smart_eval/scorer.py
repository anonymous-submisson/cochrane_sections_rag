...
class SmartScorer:
    ...
    def __init__(self, smart_types=None, matching_fn=None, split_fn=sent_tokenize):
    ...
    def _smart_1(self, score_matrix):
    ...
    def _smart_2(self, score_matrix):
    ...
    def _smart_l(self, score_matrix):
    ...
    def smart_score_precomputed(self, ref_score_matrix, src_score_matrix=None):
    ...
    def _get_score_matrix(self, tgt_sentences, can_sentences):
        ...
        pairwise_scores = [
            self.matching_fn(t, c).score for t, c in zip(tgts, cans)
        ]  # NOTE: we add `.score` here to get CHRFScore into a float
        score_matrix = np.array(pairwise_scores).reshape(
            (len(tgt_sentences), len(can_sentences))
        )
        return score_matrix

    def smart_score(self, reference, candidate, source=None):
    ...
    def corpus_smart_score_precomputed(
        self, ref_score_matrix_list, src_score_matrix_list=None
    ):
    ...
    def corpus_smart_score(self, references, candidates, sources=None):
